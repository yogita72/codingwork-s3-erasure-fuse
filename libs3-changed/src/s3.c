...
#define VERSIONING_PREFIX "enable="
#define VERSIONING_PREFIX_LEN (sizeof(VERSIONING_PREFIX) - 1)
#define KEY_MARKER_PREFIX "keyMarker="
#define KEY_MARKER_PREFIX_LEN (sizeof(KEY_MARKER_PREFIX) - 1)
#define VERSION_ID_MARKER_PREFIX "versionIdMarker="
#define VERSION_ID_MARKER_PREFIX_LEN (sizeof(VERSION_ID_MARKER_PREFIX) - 1)
                                    ...
                                    int64_t creationDate, void *callbackData)
{
	...
	data->count++ ;
	if( data->s3FileInfoList == NULL ) {
		data->s3FileInfoList =  (s3_file_info * )malloc ( 10 * sizeof(s3_file_info) );  
		data->listSize = 10;
	} else if (data->count > data->listSize) {
		data->s3FileInfoList =  (s3_file_info * )realloc (data->s3FileInfoList,  (data->listSize +10) * sizeof(s3_file_info) );  
		data->listSize += 10;
	}
   ...
   (*((s3_file_info*)&(data->s3FileInfoList[(data->count)-1]))).name = strdup(bucketName);
   (*((s3_file_info*)&(data->s3FileInfoList[(data->count)-1]))).time = creationDate ;
   (*((s3_file_info*)&(data->s3FileInfoList[(data->count)-1]))).size = -1 ;
...
int list_service(int allDetails, int *pCount, s3_file_info** pS3FileInfoList)
{
    ...
    if (statusG == S3StatusOK) {
		...
		*pCount = data.count;
		*pS3FileInfoList = data.s3FileInfoList;
    }
    else {
        printError();
    }
...
int delete_bucket(int argc, char **argv, int optindex)
{
    if (optindex == argc) {
        fprintf(stderr, "\nERROR: Missing parameter: bucket\n");
        usageExit(stderr);
                                   ...
                                   void *callbackData)
{
	...
	if( data->s3FileInfoList == NULL ) {
		data->s3FileInfoList =  (s3_file_info * )malloc ( contentsCount * sizeof(s3_file_info) );  
	} else {
		data->s3FileInfoList =  (s3_file_info * )realloc (data->s3FileInfoList,  (data->keyCount +contentsCount) * sizeof(s3_file_info) );  
	}
    ...
    for (i = 0; i < contentsCount; i++) {
        ...
        if (0) {
            ...
            printf("\n");
   			(*((s3_file_info*)&(data->s3FileInfoList[(data->keyCount)+i]))).name = strdup(content->key);
   			(*((s3_file_info*)&(data->s3FileInfoList[(data->keyCount)+i]))).time = content->lastModified ;
   			(*((s3_file_info*)&(data->s3FileInfoList[(data->keyCount)+i]))).size = content->size ;
...
void list(int argc, char **argv, int optindex)
{
    if (optindex == argc) {
		int		count =0 ;
		s3_file_info 	*s3FileInfoList = NULL;
		int 		ret = 0 ;
        ret = list_service(0, &count, &s3FileInfoList);
        return;
    }
    ...
    if (bucketName) {
		...
		s3_file_info *s3FileInfoList = NULL;
		int				ret = 0;
        ret = list_bucket(bucketName, prefix, marker, delimiter, maxkeys, 
                    allDetails, &count, &s3FileInfoList);
    }
    else {
		int		count =0 ;
		int				ret = 0;
		s3_file_info *	s3FileInfoList= NULL;
        ret = list_service(allDetails, &count, &s3FileInfoList);
    }
}
...
int delete_object(int argc, char **argv, int optindex)
{
    ...
    *slash++ = 0;
    ...
    do {
        S3_delete_object(&bucketContext, key, versionId, 0, &responseHandler, 0);
    } while (S3_status_is_retryable(statusG) && should_retry());
    ...
    *slash++ = 0;
    ...
    do {
        S3_get_object(&bucketContext, key, &getConditions, startByte,
                      byteCount, versionId, 0, &getObjectHandler, outfile);
    } while (S3_status_is_retryable(statusG) && should_retry());
...
int set_versioning(int argc, char **argv, int optindex)
{
    ...
    while (optindex < argc) {
        char *param = argv[optindex++];
        if (!strncmp(param, VERSIONING_PREFIX, VERSIONING_PREFIX_LEN)) {
            const char *verStr = &(param[VERSIONING_PREFIX_LEN]);
            if (!strcmp(verStr, "true") || !strcmp(verStr, "TRUE") ||
                !strcmp(verStr, "yes") || !strcmp(verStr, "YES") ||
                !strcmp(verStr, "1")) {
                versioning = 1;
        fprintf(stderr, "\nversioning =%d\n", versioning);
            }
    ...
    do {
        S3_set_bucket_versioning(&bucketContext, versioning,
                                     0, &responseHandler, 0);
    } while (S3_status_is_retryable(statusG) && should_retry());

    if (statusG == S3StatusOK) {
        printf("Versioning on %s set to %s \n",
                                bucketName,
...
int get_versioning(int argc, char **argv, int optindex, char **pVersioning)
{
    ...
    do {
        S3_get_bucket_versioning(&bucketContext,
                                sizeof(versioning), versioning,
                                     0, &responseHandler, 0);
...
typedef struct list_versions_callback_data
{
    int isTruncated;
    char *nextKeyMarker;
    char *nextVersionIdMarker;
    int allDetails;
        int     contentsCount;
        S3ListVersionsContent   *contents;
} list_versions_callback_data;

static void printListVersionsHeader(int allDetails)
{
    printf("%-50s  %-50s %-2s %-20s  %-5s",
           "                       Key",
           "              Version Id",
           "isLatest", "Deleted",
           "   Last Modified", "Size");
...
static S3Status listVersionsCallback(int isTruncated,
                                   const char *nextKeyMarker,
                                   const char *nextVersionIdMarker,
                                   int contentsCount,
                                   const S3ListVersionsContent *contents,
                                   ...
                                   void *callbackData)
{
    ...
    if ((!nextKeyMarker || !nextKeyMarker[0]) && contentsCount) {
        nextKeyMarker = contents[contentsCount - 1].key;
    }
    if (nextKeyMarker) {
        data->nextKeyMarker = strdup(nextKeyMarker);

    } else {
        data->nextKeyMarker = NULL;
    }

    if (nextVersionIdMarker) {
        data->nextVersionIdMarker = strdup(nextVersionIdMarker);
    }
    else {
        data->nextVersionIdMarker = NULL;
    }
    if (contentsCount && !data->contentsCount) {
        //printListVersionsHeader(data->allDetails);
           }

        if(data->contents == NULL) {
                data->contents = (S3ListVersionsContent*)
                malloc (sizeof(S3ListVersionsContent) * (contentsCount+1));
        } else {
                data->contents = (S3ListVersionsContent*)
                realloc(data->contents,
        sizeof(S3ListVersionsContent) * (contentsCount+data->contentsCount+1));
        }
        if(data->contents == NULL) {
        ...
        fprintf(stderr, "allocating keys %p\n", data->contents);


    int i;
    for (i = 0; i < contentsCount; i++) {
        const S3ListVersionsContent *content = &(contents[i]);
        char timebuf[256];
        if (0) {
            time_t t = (time_t) content->lastModified;
            strftime(timebuf, sizeof(timebuf), "%Y-%m-%dT%H:%M:%SZ",
           ...
           strftime(timebuf, sizeof(timebuf), "%Y-%m-%dT%H:%M:%SZ", 
            ...
            printf("%-50s  %s  %d %d %s  %s", content->key, content->versionId,
                                content->isLatest, content->deleteMarker,
                                         timebuf, sizebuf);
            if (data->allDetails) {
                printf("  %-34s  %-64s  %-12s",
                       content->eTag,
        ...
        memcpy((data->contents)
                + ( data->contentsCount * sizeof(S3ListVersionsContent)),
                        contents,
                        (contentsCount * sizeof(S3ListVersionsContent) )) ;
    data->contentsCount += contentsCount;
    printf("\nCommon Prefix Count: %d\n", commonPrefixesCount);
    for (i = 0; i < commonPrefixesCount; i++) {
        printf("\nCommon Prefix: %s\n", commonPrefixes[i]);
    }
                        ...
                        S3ListVersionsContent** pContents, int *pContentsCount)
{
    ...
    list_versions_callback_data data;
        if(keyMarker) {
                data.nextKeyMarker = strdup(keyMarker);
        } else {
                data.nextKeyMarker = NULL;
        }

        if( versionIdMarker ) {
                data.nextVersionIdMarker = strdup(versionIdMarker);
        } else {

                data.nextVersionIdMarker = NULL;
        }
    data.contentsCount = 0;
    data.contents = NULL;
    data.allDetails = allDetails;
    ...
    do {
        data.isTruncated = 0;
        do {
            S3_list_versions(&bucketContext, prefix,
                        data.nextKeyMarker, data.nextVersionIdMarker,
                           delimiter, maxkeys, 0, &listVersionsHandler, &data);
        } while (S3_status_is_retryable(statusG) && should_retry());
        if (statusG != S3StatusOK) {
            break;
        }
    } while (data.isTruncated && (!maxkeys || (data.contentsCount < maxkeys)));



        *pContents = data.contents ;
        *pContentsCount = data.contentsCount;
        fprintf(stderr, "list_versions contents %p\n", data.contents);

    if (statusG == S3StatusOK) {
        if (!data.contentsCount) {
            printListBucketHeader(allDetails);
        }
    }
    else {
...
static void parse_args_and_list_versions (int argc, char **argv, int optindex )
{
        S3ListVersionsContent   *contents = NULL;
        int                     contentsCount = 0 ;

    const char *bucketName = 0;

    const char *prefix = 0, *keyMarker = 0, *delimiter = 0;
    const char *versionIdMarker = 0;
    ...
    while (optindex < argc) {
        ...
        else if (!strncmp(param, VERSION_ID_MARKER_PREFIX,
                                VERSION_ID_MARKER_PREFIX_LEN)) {
            versionIdMarker = &(param[VERSION_ID_MARKER_PREFIX_LEN]);
        }
        else if (!strncmp(param, DELIMITER_PREFIX, DELIMITER_PREFIX_LEN)) {
            delimiter = &(param[DELIMITER_PREFIX_LEN]);
        }
        ...
        list_versions(bucketName, prefix, keyMarker, versionIdMarker,
                        delimiter, maxkeys, allDetails,
...
int init(int argc, char **argv)
{
    ...
    else if (!strcmp(command, "setversioning")) {
        set_versioning(argc, argv, optind);
    }
    else if (!strcmp(command, "getversioning")) {
        get_versioning(argc, argv, optind, &versioning);
    }
    else if (!strcmp(command, "listversions")) {
        parse_args_and_list_versions(argc, argv, optind);
    }
