                   ...
                   const S3GetObjectHandler *handler, void *callbackData)
{
        ...
        if( versionId) {
                subResource = (char *) malloc( strlen("versionId=")
                                        + strlen(versionId) + 5 );

                if (!subResource) {
                (*(handler->responseHandler.completeCallback))(S3StatusOutOfMemory, 0, callbackData);
                return;
                }
                sprintf(subResource, "versionId=%s", versionId);
        }
