...
int decode (int argc, char **argv) {
	...
	cs2 = strrchr(argv[1], '/');
	if (cs2 != NULL) {
		fprintf(stderr, "slash in argv\n");	
		cs2++;
		strcpy(cs1, cs2);
	}
	else {
		fprintf(stderr, "no slash in argv\n");	
		strcpy(cs1, argv[1]);
	}
	cs2 = strchr(cs1, '.');
	if (cs2 != NULL) {
	...
	fname = strchr(argv[1], '.');
	strcpy(cs2, fname);
	fname = (char *)malloc(sizeof(char*)*(1000+strlen(argv[1])+10));
	...
	fp = fopen(fname, "rb");
	temp = (char *)malloc(sizeof(char)*(strlen(argv[1])+1000));
	fscanf(fp, "%s", temp);	
	...
	while (n <= readins) {
		...
		for (i = 1; i <= k; i++) {
			...
			sprintf(fname, "%s/Coding/%s_k%0*d", curdir, cs1, md, i );
			fp = fopen(fname, "rb");
			if (fp == NULL) {
				erased[i-1] = 1;
		...
		sprintf(fname, "%s/Coding/%s_decoded", curdir, cs1);
		if (n == 1) {
			fp = fopen(fname, "wb");
		}
		else {
